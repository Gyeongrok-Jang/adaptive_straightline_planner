  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  nav2_util::declare_parameter_if_not_declared(node_, name_ + ".inflation_penalty", rclcpp::ParameterValue(100.0));
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // ---------------------------------------------------------
  // 1. Generate Manhattan-style (직각) path as guide
  // ---------------------------------------------------------
  ...
  auto guidePenalty = [&](int x, int y){
    ...
    return min_dist * cost_weight_;
  };

  auto safeDistancePenalty = [&](int x,int y){
    unsigned char c = costmap_->getCost(x,y);
    double penalty = 0.0;
    if(c >= nav2_costmap_2d::LETHAL_OBSTACLE) penalty = 1e6;
    else if(c > 0) penalty = static_cast<double>(c)/252.0 * inflation_penalty_;

    double min_obstacle_dist = std::numeric_limits<double>::max();
    for(int ix=0; ix<W; ix++){
      for(int iy=0; iy<H; iy++){
        unsigned char cv = costmap_->getCost(ix, iy);
    ...
    double safe_dist_cells = 1.0 / costmap_->getResolution();
    if(min_obstacle_dist < safe_dist_cells)
      penalty += (safe_dist_cells - min_obstacle_dist) * inflation_penalty_;
  ...
  // ---------------------------------------------------------
  // 4. Run A* with guide + safe distance penalty
  // ---------------------------------------------------------
  bool found=false;
  while(!open.empty()){
    ...
    for(int k=0;k<4;k++){
      ...
      double penalty = guidePenalty(vx, vy) + safeDistancePenalty(vx, vy);
