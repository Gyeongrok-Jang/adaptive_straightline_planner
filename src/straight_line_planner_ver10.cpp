  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  try {
    ...
    if (std::abs(robot_x - best_mid_x) < 0.20) {
      RCLCPP_WARN(node_->get_logger(),
                  "Robot is close to mid_x candidate (%.2f), using robot_x (%.2f) instead",
                  best_mid_x, robot_x);
      best_mid_x = robot_x;
    }
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  auto hfun = [&](int x,int y){ return heuristic_weight_ * (std::abs(x-gx)+std::abs(y-gy)); };
